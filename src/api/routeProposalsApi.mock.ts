...
import { RouteApiError, type RouteRequest, type RouteProposal } from '../types/route'
...
function generateMockGeometry(start: { lat: number; lng: number }, end: { lat: number; lng: number }): [number, number][] {
  // Generate a simple route with a few waypoints to simulate a real route
...
export async function fetchRouteProposalsMock(request: RouteRequest): Promise<RouteProposal[]> {
  ...
  // Generate 2-3 mock proposals with slight variations
  const proposals: RouteProposal[] = [
    {
      route: {
        geometry: {
          type: 'LineString',
          coordinates: generateMockGeometry(start, end),
        },
    ...
    },
    {
      route: {
        ...
        distance_m: Math.round(distance_m * 1.1),
        duration_s: Math.round(duration_s * 0.9),
      },
      price: {
